...
export function TerminalPanel({ isOpen, language = 'fa', messages = [], onToggle }: Props) {
  ...
  // Keep selectedCodeId pointing to a valid item
  useEffect(() => {
  ...
  const handleCopyAll = () => {
    ...
    if (codeHistory.length > 0) {
      parts.push('=== CODE ARTIFACTS ===\n');
      codeHistory.forEach((c) => {
        parts.push(`// FILE: ${c.filePath} (${new Date(c.timestamp).toISOString()})\n${c.code}\n`);
      });
    }
  ...
  return (
    ...
    >
      ...
      <div className="px-3.5 sm:px-4 py-2.5 bg-[#0d1424] text-slate-300 flex flex-wrap items-center justify-between gap-2 border-b border-white/10 shrink-0 z-20">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex items-center gap-1.5 mr-1">
            ...
            <span className="w-2.5 h-2.5 rounded-full bg-[#facc15] shadow-[0_0_6px_rgba(250,204,21,0.5)]" />
            <span className="w-2.5 h-2.5 rounded-full bg-[#34d399] shadow-[0_0_6px_rgba(52,211,153,0.5)]" />
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="flex items-center gap-1 text-[10px] text-emerald-400 font-sans font-bold bg-emerald-950/60 px-2 py-0.5 rounded-full border border-emerald-800/40">
              ...
              {totalEntries > 0 ? (isFa ? `${totalEntries} مورد` : `${totalEntries} logs`) : (isFa ? 'آماده اجرا' : 'Ready')}
            </span>
          </div>
        </div>
        ...
        {/* Right: Global Actions & Close Button */}
      ...
      {/* Single Scrollable Stream Area - ONLY ONE VERTICAL SCROLLBAR */}
      <div className="flex-1 overflow-y-auto overflow-x-hidden p-3 sm:p-4 bg-[#070b16] text-slate-200 space-y-4 leading-relaxed min-h-0 select-text font-mono">
        {totalEntries === 0 ? (
          /* Clean Empty Initial State */
          <div className="flex flex-col items-center justify-center h-full min-h-[260px] text-center text-slate-400 gap-3 py-8 px-4" dir={isFa ? 'rtl' : 'ltr'}>
            ...
            <div className="space-y-1 max-w-md">
              ...
              <p className="text-xs text-slate-400 leading-relaxed font-sans">
                {isFa
                  ? 'هنوز کدی تولید یا تغییر نیافته است. با ارسال دستور جدید به کدگر، تمام کدهای ساخته‌شده، تاریخچه تغییرات فایل‌ها و دستورات اجرا شده در یک کادر یکپارچه قرار می‌گیرند.'
                  : 'No code generated yet. Whenever you ask Codgar to build or update a component, full code outputs and execution history will appear right here in a single unified view.'}
              </p>
            </div>
          ...
          <>
            {/* 1. SINGLE UNIFIED CODE CONTAINER (تو یه دونه کادر، تقسیم‌بندی شده) */}
            {(activeTab === 'all' || activeTab === 'code') && activeCodeItem && (
              <div className="rounded-xl bg-[#090f1d] border border-emerald-500/30 overflow-hidden shadow-2xl transition-all">
                {/* File / Version Segmentation Tabs Bar */}
                <div className="px-3 py-2 bg-[#0d1527] border-b border-white/10 flex flex-wrap items-center justify-between gap-2">
                  {/* Segmented Code Tabs */}
                  <div className="flex items-center gap-1.5 overflow-x-auto py-0.5 max-w-full">
                    <span className="text-[11px] font-sans font-bold text-slate-400 flex items-center gap-1 shrink-0 mr-1" dir={isFa ? 'rtl' : 'ltr'}>
                      ...
                      <span>{isFa ? 'فایل‌ها و خروجی‌ها:' : 'Files:'}</span>
                    </span>
                    ...
                    {codeHistory.map((item, idx) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-mono transition cursor-pointer shrink-0 ${
                            isSelected
                        ...
                        >
                          ...
                          <span className="truncate max-w-[140px] sm:max-w-[200px]">{item.filePath || item.title || `Code ${idx + 1}`}</span>
                          <span className={`text-[10px] px-1 py-0.2 rounded font-sans ${isSelected ? 'bg-emerald-950/80 text-emerald-300' : 'bg-black/30 text-slate-500'}`}>
                            {item.linesCount}L
                          </span>
                        </button>
                      );
                  ...
                  <div className="flex items-center gap-2 shrink-0 ml-auto">
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-[11px] font-sans flex items-center gap-1.5 transition cursor-pointer border ${
                        wrapLines
                          ? 'bg-cyan-500/20 text-cyan-300 border-cyan-500/40 font-semibold'
                          : 'bg-white/5 text-slate-400 hover:text-white border-white/10'
                      }`}
                      title={isFa ? 'شکست خودکار خطوط برای جلوگیری از اسکرول افقی' : 'Toggle Soft Line Wrap'}
                    >
                      <WrapText className="w-3.5 h-3.5" />
                      <span>{wrapLines ? (isFa ? 'شکست خطوط: فعال' : 'Wrap: ON') : (isFa ? 'شکست خطوط: خاموش' : 'Wrap: OFF')}</span>
                    </button>
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-300 text-[11px] font-sans flex items-center gap-1.5 transition cursor-pointer border border-emerald-500/30 active:scale-95"
                    >
                      {copiedId === activeCodeItem.id ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                      <span>{copiedId === activeCodeItem.id ? (isFa ? 'کپی شد' : 'Copied') : (isFa ? 'کپی کد' : 'Copy Code')}</span>
                    </button>
                  </div>
                </div>

                {/* File Meta Info Bar */}
                <div className="px-3.5 py-2 bg-black/40 border-b border-white/5 flex flex-wrap items-center justify-between gap-2 text-xs">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-white text-xs">{activeCodeItem.filePath}</span>
                    <span className="text-[10px] text-emerald-400 bg-emerald-950/70 px-2 py-0.5 rounded border border-emerald-800/40 flex items-center gap-1 font-sans">
                      <CheckCircle2 className="w-3 h-3" />
                      <span>{isFa ? 'پیاده‌سازی موفق' : 'Compiled (Exit 0)'}</span>
                    </span>
                    <span className="text-[10px] text-cyan-300 bg-cyan-950/50 px-2 py-0.5 rounded border border-cyan-800/40 font-mono uppercase">
                  ...
                  <div className="flex items-center gap-3 text-slate-400 text-[11px] font-sans">
                    <span className="flex items-center gap-1">
                      ...
                      {new Date(activeCodeItem.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                    </span>
                    <span>{activeCodeItem.linesCount} {isFa ? 'خط' : 'lines'}</span>
                  </div>
                </div>

                {/* Build Logs Banner (if available) */}
                {activeCodeItem.executionLogs && (
                  <div className="px-3.5 py-1.5 bg-black/60 border-b border-white/5 text-[10px] text-emerald-400 font-mono flex items-center gap-2">
                    <Sparkles className="w-3 h-3 text-emerald-400 shrink-0" />
                    <span className="truncate">{activeCodeItem.executionLogs.split('\n')[0]}</span>
                  </div>
                )}

                {/* Natural-Height Code View with Line Numbers - NO NESTED VERTICAL SCROLL TRAP */}
                <div className="relative bg-[#070b16] text-[11px] text-slate-200 font-mono leading-relaxed select-text flex">
                  {/* Line Numbers Gutter */}
                  <div className="select-none py-3 px-2.5 text-right text-slate-600 bg-black/40 border-r border-white/5 font-mono text-[11px] leading-6 shrink-0 min-w-[42px]">
                    {codeLines.map((_, i) => (
                      <div key={i}>{i + 1}</div>
                    ))}
                  </div>
                  ...
                  {/* Code Body */}
                  <div className="flex-1 min-w-0 py-3 px-3.5 overflow-hidden">
                    <pre
                      className={`font-mono text-[11px] leading-6 text-slate-200 ${
            ...
            {(activeTab === 'all' || activeTab === 'commands') && commandLogs.length > 0 && (
              <div className="space-y-2.5 pt-1">
                <div className="flex items-center justify-between text-xs text-slate-400 px-1 font-sans font-bold">
                  <span className="flex items-center gap-1.5">
                    <TerminalIcon className="w-3.5 h-3.5 text-cyan-400" />
                    <span>{isFa ? 'تاریخچه دستورات اجرا شده در ترمینال' : 'Executed Command Logs'}</span>
                  </span>
                  <span className="text-[10px] text-slate-500 font-normal">({commandLogs.length} {isFa ? 'دستور' : 'commands'})</span>
                </div>

                {commandLogs.map((log) => (
                  <div key={log.id} className="rounded-xl bg-black/50 p-3 border border-white/10 shadow-sm space-y-1.5">
                    <div className="flex items-center justify-between text-[11px] text-slate-400 border-b border-white/5 pb-1.5">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="flex items-center gap-1 text-slate-500 text-[10px] font-sans">
                        ...
                        >
                          ...
                          <span>{copiedId === log.id ? (isFa ? 'کپی شد' : 'Copied') : (isFa ? 'کپی' : 'Copy')}</span>
                        </button>
                      </div>
                    </div>
                    {log.stdout && (
      ...
      <div className="p-2 sm:p-2.5 bg-[#0d1424] border-t border-white/10 flex items-center gap-2 shrink-0 select-none">
