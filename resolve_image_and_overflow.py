...
# ۱. اصلاح بک‌اند جهت استخراج داینامیک پرامپت و حذف هرگونه مقدار هاردکد شده
...
if os.path.exists(route_path):
    ...
    dynamic_handler = """
    // پردازش داینامیک درخواست ساخت تصویر با موتور FLUX
...
"""
    ...
    if idx != -1:
        ...
        with open(route_path, "w", encoding="utf-8") as f:
            f.write(routes)
        print("✅ سرور برای استخراج داینامیک پرامپت آماده شد.")

# ۲. تزریق موتور رندرینگ تصویر با فونت مدرن اپل و مهار کامل کادر در index.html
...
if os.path.exists(index_path):
    ...
    (function() {
      function renderAIImages() {
        // ۱. مهار متون چت برای جلوگیری از بیرون‌زدگی
        document.querySelectorAll("p, span, div").forEach(el => {
          if (el.innerText && el.innerText.includes("image.pollinations.ai") && !el.classList.contains("chat-text-contained")) {
            el.classList.add("chat-text-contained");
          }
        });

        // ۲. تبدیل لینک تصویر به کارت گرافیکی
        const textNodes = [];
        ...
        for (const tn of textNodes) {
          ...
          if (match) {
            ...
            card.className = "ai-image-card my-2 w-full max-w-[500px] overflow-hidden rounded-2xl border border-slate-700/60 bg-slate-900/95 text-white shadow-2xl backdrop-blur-xl";
            ...
            card.innerHTML = `
              <!-- هدر با متادیتا و پرامپت تمیز درون کادر -->
              <div style="padding: 10px 14px; background: rgba(15, 23, 42, 0.75); border-bottom: 1px solid rgba(51, 65, 85, 0.6); display: flex; flex-direction: column; gap: 5px;">
                ...
                <div style="font-size: 12px; font-weight: 600; color: #f8fafc; line-height: 1.4; white-space: normal; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">
                  ${promptTitle}
                </div>
              </div>

              <!-- بدنه تصویر با لودینگ متحرک FLUX -->
              <div style="position: relative; width: 100%; aspect-ratio: 16/9; background: #020617; display: flex; align-items: center; justify-content: center; overflow: hidden;">
                <div class="loader-box" style="position: absolute; inset: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 8px; background: rgba(2, 6, 23, 0.95); color: #60a5fa; font-size: 11.5px;">
                  ...
                  <span style="font-weight: 500; letter-spacing: 0.02em;">در حال رندر و تولید زنده تصویر با موتور FLUX...</span>
...
"""
    ...
    with open(index_path, "w", encoding="utf-8") as f:
        f.write(html)
    print("✅ موتور مهار متن و رندر تصویر در index.html مستقر شد.")
...
subprocess.run(["git", "commit", "-m", "fix(ui): enforce strict text containment, Apple SF Pro typography, and live FLUX image rendering"], check=False)
